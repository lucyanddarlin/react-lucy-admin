...
export default function MultiTabs({ offsetTop = false }: Props) {
  ...
  const multiTabStyles: CSSProperties = {
    position: 'fixed',
  ...
  if (themeLayout === ThemeLayout.Horizontal) {
    multiTabStyles.top = HEADER_HEIGHT + NAV_HORIZONTAL_HEIGHT - 2;
  } else if (screenMap.md) {
    multiTabStyles.right = '0px';
    multiTabStyles.left = 'auto';
    multiTabStyles.width = `calc(100% - ${
      themeLayout === ThemeLayout.Vertical ? NAV_WIDTH : NAV_COLLAPSED_WIDTH
    }px)`;
  } else {
    multiTabStyles.width = '100vw';
  }

  const renderTabBar: TabsProps['renderTabBar'] = () => {
    return (
      <div className="z-20 w-full" style={multiTabStyles}>
  ...
  /**
   * scrollContainer 监听 mouse wheel 事件
   */
  useEffect(() => {
    function handlerMouseWheel(event: WheelEvent) {
      event.preventDefault();
      scrollContainer.current!.scrollLeft += event.deltaY;
    }

    scrollContainer.current!.addEventListener('mouseenter', () => {
      scrollContainer.current!.addEventListener('wheel', () => handlerMouseWheel);
    });
    scrollContainer.current!.addEventListener('mouseleave', () => {
      scrollContainer.current!.removeEventListener('wheel', handlerMouseWheel);
    });
  }, []);
